...
public class TodosController {
	...
	@GetMapping("/todos/bytitle")
	public List<Todos> findByTitle(@RequestParam("title") String title) {
		log.info("Todos controller : findByTitle Task executed");
		return (List<Todos>) todosRepository.findByTitle(title);

	}
	@GetMapping("/todos/byassigned")
	public List<Todos> findByAssignedBy(@RequestParam("assigned") String assigned) {
		log.info("Todos controller : findByAssignedBy Task executed");
		return (List<Todos>) todosRepository.findByAssignedBy(assigned);

	}
	@GetMapping("/todos/by/level/completed")
	public List<Todos> findByDifficultyLevelAndCompleted(@RequestParam("difficultyLevel") String difficultyLevel,
														 @RequestParam("completed")String completed) {
		log.info("Todos controller : findByDifficultyLevelAndCompleted Task executed");
